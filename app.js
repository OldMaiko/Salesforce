...
const flowSteps = [
  ...
  },
  {
    ...
    title: "Sample",
    subtitle: "送样流程",
    summary: "2026-09-09 发布待执行：PROD 部署后，在 Contact Sample Request 中统一编辑样品并确认申请，随后维护物流信息。",
...
const features = [
  ...
  },
  {
    ...
    category: "送样",
    title: "确认申请、发货与物流跟进",
    summary: "Confirm Request 是唯一的样品保存和内部确认动作；发货后维护物流并使用 Sample Fulfilled。",
    capabilities: ["Confirm Request", "Request Confirmed", "Sample Fulfilled", "Tracking"],
    actions: ["确认 Draft 的地址、非美国 Contact 电话和全部 Samples。", "只点击一次 Confirm Request。", "发货后维护 Tracking、Courier、ETA，再使用 Sample Fulfilled。"],
    automation: ["Confirm Request 后状态显示为 Request Confirmed，且没有 Add Samples 或邮件预览页面。", "AfterShip 仅补充空白 ETA/Courier；已送达记录受保护。"],
...
const capabilities = [
  ...
  },
  {
    ...
    category: "送样",
    title: "样品申请与客户确认",
    summary: "2026-09-09 发布待执行：PROD 部署后，从 Contact Sample Request 创建 Draft，统一编辑 Samples，确认后再维护配送与物流状态。",
    icon: "package-check",
    accent: "green",
...
const helpArticles = [
  ...
  },
  {
    ...
    summary: "2026-09-09 发布待执行：PROD 部署后，为客户创建 Draft，在 Sample SKUs 区域一次处理既有和新增样品。",
    prerequisites: ["目标客户已经建成 Contact。", "收件信息和业务背景已确认。"],
    steps: ["进入 Contact Sample Request 对象或相关入口。", "单击 New 创建 Draft。", "选择对应 Contact，并填写收件地址、业务背景等基础信息。", "保存主申请。", "在状态路径下方的 Sample SKUs 中搜索 SKU 或产品描述。", "可单个 Add，或选择多项后 Add Selected。", "在左侧 Samples 编辑现有或新增行的 Quantity、Usage 和 PO#。", "如需移除，先暂存 Remove；Undo 会恢复最近移除的行，重新选择 SKU 会恢复该行而不新增重复项。", "先不要在 Related tab 新增样品；完成选择后转到 Confirm Request。"],
  ...
  },
  {
    ...
    summary: "2026-09-09 发布待执行：PROD 部署后，Confirm Request 是 Draft 中唯一的编辑保存和内部确认动作。",
    prerequisites: ["Contact Sample Request 仍为 Draft。", "至少一条 Samples 已填写，并已检查地址、非美国 Contact 的 Mobile Phone、数量和业务背景。"],
    steps: ["打开目标 Draft Contact Sample Request。", "再次检查 Contact、收件信息、SKU、Quantity、Usage、PO# 和业务背景。", "若 Delivery Address 为空，先确认 Contact 的 Mailing Address 可用；非美国 Contact 还需要 Mobile Phone。", "单击一次 Confirm Request。", "系统会在同一事务中保存样品变更、发送内部确认邮件，并将状态更新为 Request Confirmed。", "刷新记录，确认编辑器已换成原生 Contact Samples 相关列表。"],
  ...
  },
  {
    ...
    summary: "2026-09-09 发布待执行：PROD 部署后，确认申请后在原生 Contact Samples 维护物流资料并用 Sample Fulfilled 记录发货。",
    prerequisites: ["送样申请状态已为 Request Confirmed。", "配送信息已经从内部同事或承运商获得。"],
    steps: ["在原生 Contact Samples 相关列表打开样品。", "填写或更新 Tracking、Shipping Courier、ETA（Date of Shipment）和需要维护的 PO#。", "发货资料准备好后，使用 Sample Fulfilled；业务状态更新为 Shipment Created。", "AfterShip 只会补充空白 ETA/Courier，不会覆盖已有值，也不会把日期范围猜成单一 ETA。", "如显示 Attention Required，查看运单最新更新并与销助处理。", "Delivered 后如需更正关键资料，请联系管理员按已送达更正流程处理；补寄建议新建 Request。"],
    verify: ["状态显示为 Shipment Created，后续物流状态可按承运商更新。", "Tracking、Courier 与 ETA 正确。", "已有 ETA/Courier 没有被自动覆盖。"],
    tips: ["自动轮询是否已启用及实际运行状态由管理员在 PROD 发布时确认；页面显示或 UAT 验收本身不等同于外部邮件投递或自动同步已经完成。"],
  },
  {
