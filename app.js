...
const flowSteps = [
  ...
  },
  {
    ...
    title: "Sample",
    subtitle: "送样流程",
    summary: "在 Contact Sample Request 中统一编辑样品并确认申请，随后维护物流信息。",
    actions: ["创建 Draft", "编辑 SKU、数量、用途和 PO#", "Confirm Request"],
    outputs: ["Request Confirmed", "Shipment Created", "Tracking / PO 信息"],
    color: "rose",
  },
  {
    id: "opportunity",
...
const existingCustomerFlowSteps = [
  ...
  },
  {
    ...
    actions: ["从对应 Contact 进入 Contact Sample Request", "补齐 SKU、数量、用途和收件信息", "根据状态跟进 Request Confirmed、Shipment Created 和 Tracking"],
...
const features = [
  ...
  },
  {
    id: "sample-request",
    category: "送样",
    title: "创建送样申请并统一编辑 Samples",
    summary: "在 Draft 的 Sample SKUs 区域处理现有和新增样品，再以 Confirm Request 一次完成保存与内部确认。",
    capabilities: ["Contact Sample Request", "Sample SKUs", "Quantity / Usage / PO#", "收件信息"],
    actions: ["先确认 Contact、收件信息和业务背景准确。", "在 Sample SKUs 编辑 SKU、数量、用途和 PO#。", "确认前处理暂存移除或 Undo。"],
    automation: ["Draft 中的编辑在 Confirm Request 时一次保存；确认后显示原生 Contact Samples 相关列表。"],
    tags: ["Sample Request", "Sample SKUs", "Confirm Request"],
    accent: "green",
  },
  {
    ...
    category: "送样",
    title: "确认申请、发货与物流跟进",
    summary: "Confirm Request 完成内部确认；Sample Fulfilled 不即时发信，由 AfterShip 首次成功查询选择 Shipment Created 或 Delivered，后续不发送 Out for Delivery 邮件。",
    capabilities: ["Confirm Request", "Request Confirmed", "Sample Fulfilled", "Tracking"],
    actions: ["确认 Draft 的地址、非美国 Contact 电话和全部 Samples。", "只点击一次 Confirm Request。", "发货后维护 Tracking、Courier、ETA；可先履行已有 Tracking 的部分样品。"],
    automation: ["单击 Sample Fulfilled 时不发邮件，等待 AfterShip 首次成功查询：首次已 Delivered 就直接发送 Delivered，其他状态发送 Shipment Created。", "后续 Out for Delivery 不发邮件；后续 Delivered 按 Tracking 通知客户。NA Sales Team Public Group 成员创建的 Request 不发送任何邮件，但状态仍正常更新。"],
    tags: ["Request Confirmed", "Shipment Created", "AfterShip"],
    accent: "orange",
  },
  {
...
const capabilities = [
  ...
  },
  {
    ...
    accent: "green",
    screenshot: "fig38",
    solves: ["把口头送样沟通变成系统里能追溯的记录。", "在一个编辑区管理 SKU、数量、用途、PO 和收件信息。", "用 Confirm Request 完成唯一的保存和内部确认动作。"],
    when: ["客户需要样品评估。", "导入字段自动生成了 Sample，需要后续正式申请。", "内部已发货，需要维护 Tracking 和物流状态时。"],
    articleIds: ["check-sample-auto-records", "create-sample-request", "submit-sample-request", "shipping-customer-confirmation"],
    tags: ["Sample Request", "Sample SKUs", "Tracking", "Request Confirmed"],
  },
  {
    id: "opportunity",
    category: "商机",
...
const helpArticles = [
  ...
  },
  {
    ...
    category: "送样",
    title: "创建 Contact Sample Request 并编辑 Samples",
    summary: "为客户创建 Draft，在 Sample SKUs 区域一次处理既有和新增样品。",
    screenshots: ["fig38"],
    prerequisites: ["目标客户已经建成 Contact。", "收件信息和业务背景已确认。"],
    steps: ["进入 Contact Sample Request 对象或相关入口。", "单击 New 创建 Draft。", "选择对应 Contact，并填写收件地址、业务背景等基础信息。", "保存主申请。", "在状态路径下方的 Sample SKUs 中搜索 SKU 或产品描述。", "可单个 Add，或选择多项后 Add Selected。", "在左侧 Samples 编辑既有或新增行的 Quantity、Usage 和 PO#。", "如需移除，先暂存 Remove；Undo 会恢复最近移除的行，重新选择 SKU 会恢复该行而不新增重复项。", "完成编辑后保持当前页面，按下一篇指南检查并点击 Confirm Request；确认前刷新或离开会丢失未提交编辑。"],
    verify: ["Contact Sample Request 主记录已创建且仍是 Draft。", "Contact、收件信息、SKU 和数量正确。", "至少保留一条样品。"],
    tips: ["Contact 和货币由当前 Request 继承。新增加的 SKU 会继承最近输入的 Usage，但每一行都可修改。"],
  },
  {
    id: "submit-sample-request",
    category: "送样",
    title: "用 Confirm Request 完成保存与内部确认",
    summary: "Confirm Request 是 Draft 中唯一的编辑保存和内部确认动作。",
    prerequisites: ["Contact Sample Request 仍为 Draft。", "至少一条 Samples 已填写，并已检查地址、非美国 Contact 的 Mobile Phone、数量和业务背景。"],
    steps: ["打开已完成样品编辑的 Draft Contact Sample Request。", "核对 Contact、收件信息、业务背景及每行已编辑的 Quantity、Usage、PO#。", "若 Delivery Address 为空，先确认 Contact 的 Mailing Address 可用；非美国 Contact 还需要 Mobile Phone。", "确认至少保留一条 Samples 后，单击一次 Confirm Request。", "系统会在同一事务中保存样品变更、发送内部确认邮件，并将状态更新为 Request Confirmed。若 Request 创建人属于 NA Sales Team Public Group，系统只更新记录，不发送任何邮件。", "刷新记录，确认编辑器已换成原生 Contact Samples 相关列表。"],
    verify: ["状态显示为 Request Confirmed。", "所有新增、更新或明确暂存移除的样品都符合预期。", "不显示 Add Samples、邮件预览页面或旧的 Email Confirmation (Ext.) 步骤。"],
    tips: ["若 Request/样品在编辑期间已变更、确认失败或邮件失败，系统会回滚本次样品变更；刷新后按错误提示处理，避免重复确认。"],
  },
  {
    ...
    screenshots: ["emailShipmentCreated", "emailAttentionRequired", "emailDelivered"],
    prerequisites: ["送样申请状态已为 Request Confirmed。", "配送信息已经从内部同事或承运商获得。"],
    steps: ["在原生 Contact Samples 相关列表打开样品。", "填写或更新 Tracking、Shipping Courier、ETA（Date of Shipment）和需要维护的 PO#。SKU 会完整显示、长名称自动换行，可打开产品链接；Quantity 以整数显示。", "至少一行已有 Tracking 后，使用 Sample Fulfilled；系统只处理有 Tracking 的行，但此时不发送邮件，必须等待 AfterShip 首次成功查询。未填 Tracking 的行会保留，允许后续部分履行。", "AfterShip 首次成功查询时：如果运单已经 Delivered，系统跳过 Shipment Created，直接发送 Delivered；其他任何状态都发送一次 Shipment Created。", "首次查询之后，Out for Delivery 不再发送邮件；后续变为 Delivered 时，按 Tracking 发送 Delivered 通知。只有同一 Request 的全部样品均 Delivered，Request 才变为 Delivered。", "AfterShip 只在 API 有返回时补充空白 ETA/Courier，不覆盖已有值。", "如后续显示 Attention Required，查看对应运单的最新更新并与销助处理；此类邮件只发给内部销售和支持人员。", "如果 Request 创建人属于 NA Sales Team Public Group，该 Request 的确认、Shipment Created、Attention Required 和 Delivered 等邮件全部不发送，但记录与物流状态仍会正常更新。"],
    verify: ["单击 Sample Fulfilled 后没有即时邮件；AfterShip 首次成功查询后才按查询结果选择 Shipment Created 或 Delivered。", "首次查询已经 Delivered 时只发送 Delivered；后续 Out for Delivery 不发送邮件，后续 Delivered 才发送 Delivered 通知。", "NA Sales Team 成员创建的 Request 不发送任何邮件，但 Tracking、Courier、ETA 和状态仍正常更新。", "Tracking、Courier 与 ETA 正确，SKU 可完整阅读，Quantity 为整数，且已有 ETA/Courier 没有被自动覆盖。"],
    tips: ["AfterShip 启用后每小时同步。邮件判断以首次成功取得的 AfterShip 状态为准；查询失败时不会提前发送 Shipment Created。没有客户邮箱或 Request 创建人属于 NA Sales Team Public Group 时，系统仍会更新状态，但不会发送相应邮件。系统不会把日期范围猜成单一 ETA。Delivered 后如需更正关键资料，请联系管理员；补寄请新建 Request。如物流长时间未更新，请联系管理员检查同步任务和凭据。"],
  },
  {
...
const rules = [
  ...
  {
    title: "送样申请状态按钮",
    summary: "Draft 的 Sample SKUs 编辑器和确认动作按状态切换。",
    bullets: ["Draft 可编辑 Samples 并使用 Confirm Request。", "Confirm Request 是唯一的样品保存和内部确认动作；没有 Add Samples 或邮件预览页面。", "Request Confirmed 后显示原生 Contact Samples 相关列表；发货时使用 Sample Fulfilled。"],
  },
  {
