...
const flowSteps = [
  ...
  },
  {
    ...
    title: "Email",
    subtitle: "客户触达",
    summary: "通过列表邮件或 Campaign 邮件发送个性化内容，并把客户互动沉淀到 Activity。",
    actions: ["确认筛选条件和收件人数", "使用合并字段", "发送测试邮件"],
    outputs: ["Activity 记录", "Initial contact 状态", "可追踪触达历史"],
    color: "teal",
  },
  {
    id: "status",
    title: "Lead Status",
    subtitle: "状态推进",
    summary: "从 New 到 Initial contact、Engaged、Converted，状态驱动看板和后续转化判断。",
    actions: ["查看 Lead Status 看板", "用 View Report 钻取明细", "及时转化有价值 Lead"],
  ...
  },
  {
    ...
    title: "Review",
    subtitle: "数据复盘",
    summary: "把 Campaign、客户互动、样品、Opportunity 连接起来，复盘展会投入到成交结果。",
    actions: ["计算 Campaign Results", "刷新嵌入式看板", "检查 Campaign 与 Opportunity 是否对齐"],
...
const features = [
  {
    ...
    category: "入门",
    title: "快速入门与每日节奏",
    summary: "帮助销售用户先掌握核心对象、Owner、Activity 和状态更新的工作习惯。",
    screenshots: ["fig01"],
    capabilities: ["先搜索后新建", "Owner 责任人", "Activity 跟进证据", "每日 60 秒检查"],
    actions: ["开始工作先看 Task、逾期提醒、重点 Opportunity 和 Key Account。", "每次客户互动后更新 Lead Status 或 Opportunity Stage。", "结束工作前创建下一步 Task 并检查 Campaign 关联。"],
  ...
  },
  {
    ...
    summary: "把实际 Salesforce 中的 Home、Leads、Contacts、Accounts、Opportunities、Campaigns、Dashboards、Reports、Forecasts 和自定义对象串成工作入口。",
    screenshots: ["fig04", "fig05"],
    capabilities: ["全局搜索", "对象导航", "More 更多菜单", "Edit nav items", "Favorites"],
    actions: ["先用全局搜索查找已有客户、公司或商机，避免重复创建。", "常用对象直接从顶部导航进入；低频对象从 More 中进入。", "把高频页面加入 Favorites，或用 Edit nav items 调整自己的导航顺序。"],
    automation: ["顶部导航只改变入口，不改变数据；但进入正确对象会影响你后续创建记录和关联关系的准确性。"],
  ...
  },
  {
    ...
    category: "列表",
    title: "列表视图与新建记录",
    summary: "从顶部导航进入对象，切换视图、理解筛选条件，并快速新建记录。",
  ...
  },
  {
    ...
    category: "列表",
    title: "Leads 列表页批量操作",
    summary: "在实际 Leads 列表中使用 New、Import、Add to Campaign、Change Status、Change Owner、Search this list 和列排序。",
  ...
  },
  {
    ...
    summary: "面向 Lead 或 Contact 列表进行批量邮件触达，并用 Kanban 查看状态分布。",
  ...
  },
  {
    ...
    summary: "Salesforce 记录销售与客户之间的邮件往来，把完整内容沉淀在 Lead 或 Contact 的 Activity 模块。",
    screenshots: ["fig25"],
  ...
  },
  {
    ...
    summary: "从 Campaign 给成员发送列表邮件，利用合并字段个性化内容，并驱动 Lead Status 变化。",
    screenshots: ["fig26", "fig27", "fig28", "fig29"],
    capabilities: ["Campaign 列表邮件", "合并字段", "Lead Status 看板", "View Report 明细"],
    actions: ["发送前预览或发送测试邮件。", "使用合并字段插入客户姓名等信息。", "通过 View Report 查看底层明细。"],
  ...
  },
  {
    ...
    summary: "当 Lead 有明确合作意向、预算或推进价值时，将其转化为 Contact、Account 和 Opportunity。",
  ...
  },
  {
    ...
    actions: ["命名规则使用 Company / 展会名 / 年份。", "维护 Contact Roles、Partners 和关键客户关系。", "Opportunity 必须关联正确 Campaign 才能形成闭环。"],
  ...
  },
  {
    ...
    summary: "从实际 Sales App 导航进入看板、报表和预测页面，用统一口径查看销售结果、明细和预测。",
    screenshots: ["fig02", "fig22", "fig29"],
    capabilities: ["Dashboards", "Reports", "Forecasts", "View Report", "Refresh"],
    actions: ["用 Dashboards 看趋势和汇总。", "用 Reports 钻取明细，确认底层记录是否符合筛选条件。", "用 Forecasts 查看预测视角，并与 Opportunity 阶段和金额对齐。"],
  ...
  },
  {
    ...
    actions: ["在 List Emails 中回看列表邮件发送记录和状态。", "报价或产品信息需要结构化沉淀时，进入 Shokz Quote 或 Shokz Product。", "有明确审计任务时，从 More 进入 Sales Data Audit Export，并先确认导出范围。", "底部工具栏用于临时笔记、历史页面和待办追踪。"],
    automation: ["List Emails 与 Activity、Campaign 邮件触达有关；Quote、Product、Audit Export 和 To Do 信息需要与真实客户或商机上下文保持一致。"],
  ...
  },
  {
    ...
    category: "复盘",
    title: "数据闭环与展会复盘",
    summary: "将 Campaign、客户互动、样品和 Opportunity 连接起来，复盘展会投入到成交结果。",
...
const rules = [
  ...
  },
  {
    ...
    bullets: ["Account 及其 Contacts 最近 30 天无已完成 Task、Email 或 Event 时触发。", "系统周一和周五提醒 Owner，并确保存在待处理跟进任务。"],
  },
];
...
const glossary = [
  ...
  ["Report", "报表", "可筛选、分组和导出的底层记录明细"],
  ["Forecast", "预测", "基于商机金额、阶段和预计成交日期形成的销售预测"],
  ["List Email", "列表邮件", "面向多个 Lead 或 Contact 发送并可追踪的邮件记录"],
...
function renderHelpArticleDetail(article) {
  if (!article) {
    els.helpArticleDetail.innerHTML = `
      <div class="no-results">没有匹配的帮助文章。可以换个关键词，例如 List View、Campaign Results、Samples Sent 或 Convert。</div>
    `;
    return;
  }
...
function renderHelpCenter() {
  ...
  els.helpCount.textContent = `${filtered.length} 篇文章`;
  if (!filtered.length) {
    els.helpArticleList.innerHTML = `<div class="no-results">没有匹配的文章。</div>`;
...
function renderFeatures() {
  ...
  els.featureCount.textContent = `${filtered.length} 个能力`;
  if (!filtered.length) {
    els.featureGrid.innerHTML = `<div class="no-results">没有匹配的能力。换个关键词试试，比如 Campaign、送样、Forecasts 或 Opportunity。</div>`;
    return;
  }
        ...
        <article class="capability-card ${feature.accent}">
          ...
          <div class="capability-footer">
            ...
            <div class="capability-actions">
              <span>${feature.articleIds.length} 篇相关文章</span>
              <a href="capabilities/${feature.id}.html">打开能力页</a>
            </div>
          </div>
        </article>
      `;
