...
const capabilities = [
  ...
  },
  {
    ...
    category: "送样",
    title: "样品申请与客户确认",
    summary: "从 Contact Sample Request 创建 Draft，统一编辑 Samples，确认后再维护配送与物流状态。",
    icon: "package-check",
    accent: "green",
    screenshot: "fig38",
...
const helpArticles = [
  ...
  },
  {
    ...
    steps: ["进入 Contact Sample Request 对象或相关入口。", "单击 New 创建 Draft。", "选择对应 Contact，并填写收件地址、业务背景等基础信息。", "保存主申请。", "在状态路径下方的 Sample SKUs 中搜索 SKU 或产品描述。", "可单个 Add，或选择多项后 Add Selected。", "在左侧 Samples 编辑既有或新增行的 Quantity、Usage 和 PO#。", "如需移除，先暂存 Remove；Undo 会恢复最近移除的行，重新选择 SKU 会恢复该行而不新增重复项。", "完成后只核对 Samples 是否齐全，保留该 Draft 供下一篇指南确认。"],
  ...
  },
  {
    ...
    steps: ["打开已完成样品编辑的 Draft Contact Sample Request。", "核对 Contact、收件信息、业务背景及每行的 Quantity、Usage、PO#；不在此步骤新增或移除 SKU。", "若 Delivery Address 为空，先确认 Contact 的 Mailing Address 可用；非美国 Contact 还需要 Mobile Phone。", "确认至少保留一条 Samples 后，单击一次 Confirm Request。", "系统会在同一事务中保存样品变更、发送内部确认邮件，并将状态更新为 Request Confirmed。", "刷新记录，确认编辑器已换成原生 Contact Samples 相关列表。"],
  ...
  },
  {
    ...
    summary: "确认申请后在原生 Contact Samples 维护物流资料并用 Sample Fulfilled 记录发货。",
